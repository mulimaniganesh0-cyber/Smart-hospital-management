...
class HospitalProvider extends ChangeNotifier {
 ...
 Future<void> loadHospitalData() async {
  ...
  try {
    ...
    try {
      ...
      if (emergenciesResponse['success'] == true && emergenciesResponse['data'] != null) {
  ...
  Future<bool> updateResource(String resourceType, String field, int value) async {
    _isLoading = true;
    _notify();
 ...
 // lib/providers/hospital_provider.dart
  ...
  Future<bool> updateAppointmentStatus(int appointmentId, String status) async {
    ...
    try {
      final response = await ApiService.updateAppointmentStatus(appointmentId, status);
      if (response['success'] == true) {
        await loadHospitalData();
        return true;
      }
...
Future<bool> addBloodStock(String bloodGroup, int units, String expiryDate) async {
  _isLoading = true;
  _notify();
...
Future<bool> deleteBloodStock(String bloodGroup) async {
  ...
  Future<bool> addNewResource(String resourceType, int total, int available) async {
    _isLoading = true;
    _notify();
