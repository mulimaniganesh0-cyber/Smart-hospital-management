...
exports.deleteHospital = async (req, res) => {
  ...
  try {
    ...
    const hospitalCheck = await client.query(
      'SELECT id, name, email FROM hospitals WHERE id = $1',
      [hospitalId]
    );
    ...
    // 1. Delete blood bank records
    await client.query(
      'DELETE FROM blood_bank WHERE hospital_id = $1',
      [hospitalId]
    );

    // 2. Delete ambulance bookings
    await client.query(
      'DELETE FROM ambulance_bookings WHERE hospital_id = $1',
      [hospitalId]
    );

    // 3. Delete emergency requests
    await client.query(
      'DELETE FROM emergency_requests WHERE hospital_id = $1',
      [hospitalId]
    );

    // 4. Delete appointments
    await client.query(
      'DELETE FROM appointments WHERE hospital_id = $1',
      [hospitalId]
    );

    // 5. Delete resource requests
    await client.query(
      'DELETE FROM resource_requests WHERE hospital_id = $1',
      [hospitalId]
    );

    // 6. Delete blood requests
    await client.query(
      'DELETE FROM blood_requests WHERE hospital_id = $1',
      [hospitalId]
    );

    // 7. Delete hospital resources
    await client.query(
      'DELETE FROM hospital_resources WHERE hospital_id = $1',
      [hospitalId]
    );

    // 8. Delete hospital staff
    await client.query(
      'DELETE FROM staff WHERE hospital_id = $1',
      [hospitalId]
    );

    // 9. Finally delete the hospital
    await client.query(
    ...
    console.log(`✅ Hospital "${hospital.name}" (ID: ${hospitalId}) deleted successfully`);
